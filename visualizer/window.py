...
class Window:
    ...
    def draw_traffic_lights(self):
        for traffic_light, segment_index in self.simulation.traffic_lights:
            segment = self.simulation.segments[segment_index]
            position = segment.get_point(1)  
            color = (255, 0, 0) if traffic_light.is_red() else (0, 255, 0)
            dpg.draw_circle(center=position, radius=1 * self.zoom, color=color, fill=color, parent="Canvas")
